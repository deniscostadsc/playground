...
int main() {
    ...
    while (n--) {
        ...
        if (down && !none) std::cout << "DOWN";
        if (down && left && !none) std::cout << " ";
        if (left && !none) std::cout << "LEFT";
        if ((down || left) && right && !none) std::cout << " ";
        if (right && !none) std::cout << "RIGHT";
        if ((down || left || right) && up && !none) std::cout << " ";
        if (up && !none) std::cout << "UP";
        if ((!down && !up && !left && !right) || none) std::cout << "NONE";
        std::cout << std::endl;
    }
    return 0;
}
